...
def wbf(bboxes_coord, bboxes_cat, bboxes_rad, nms_iou=0.4, image_size=None):
...
def nms_svcat(bboxes_coord, bboxes_cat, bboxes_rad=None, nms_iou=0.5, image_size=None):
    bbox_sizes = np.array([(x1 - x0) * (y1 - y0) for (x0, y0, x1, y1) in bboxes_coord])
    order = bbox_sizes.argsort()  # [::-1]
    keep = [True] * len(order)
