...
class FocalLoss(nn.Module):
    ...
    def forward(self, outputs, annotations):
        ...
        classifications = outputs["classification"]
        regressions = outputs["regression"]
        ...
        for j in range(batch_size):
            ...
            alpha_factor = torch.where(
                torch.ge(targets, 0.5), alpha_factor, 1.0 - alpha_factor
            )
            focal_weight = torch.where(
                torch.gt(targets, 0.5), 1.0 - classification, classification
            )
