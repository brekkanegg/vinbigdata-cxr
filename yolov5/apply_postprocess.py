...
def lungseg_filtering(strings, uid, vote=False, dilation=True):
    try:
        lungseg = cv2.imread(LUNGSEG_DIR + f"/{uid}_abdomen.png") // 255

        if dilation:
            lungseg = cv2.dilate(
                lungseg, kernel=np.ones((3, 3), np.uint8), iterations=1
            )
...
def get_best_one(strings, targets=[0, 3]):
    if len(strings) < 2:
        return strings
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--lungseg_dir",
        type=str,
        default="/data/minki/kaggle/vinbigdata-cxr/final_segmap",
    )
    ...
    global LUNGSEG_DIR
    LUNGSEG_DIR = opt.lungseg_dir

    seg_out, rule_out, clswise_wbf = True, True, True
    nb_boxes, nb_segout, nb_ruledout, nb_wbfeffect = 0, 0, 0, 0
    nb_segout = 0
    for idx_lbl, lbls in enumerate(target_labels):
        ...
        if clswise_wbf:
            strings = apply_classwise_wbf(strings)
