...
class Trainer(object):
    ...
    def train_val_one_epoch(self):
        ...
        for i, data in enumerate(self.train_loader):
            ...
            img = torch.FloatTensor(data[1]).permute(0, 3, 1, 2).to(self.device)

            # TODO: reformat data target
            target = {}
            target["cls"] = [
                torch.FloatTensor(data[2][i][:, -1]).to(self.device)
                for i in range(len(img))
            ]
            target["bbox"] = [
                torch.FloatTensor(data[2][i][:, [1, 0, 3, 2]]).to(self.device)
                for i in range(len(img))
            ]  # xyxy --> yxyx
        ...
        if self.epoch > self.cfgs["model"]["val"]["ignore_epoch"]:
            ...
            # Save Model
            if val_best > self.tot_val_record["best"]["coco"]:
                val_improved = True
