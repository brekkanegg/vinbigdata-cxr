...
class DetectionFastCollate:
    ...
    def __init__(
        ...
        instance_shapes = instance_shapes or dict(
            bbox=(max_instances, 4),
            bbox_ignore=(max_instances, 4),
            cls=(max_instances,),
        )
        self.instance_info = {
            k: dict(fill=instance_fill, shape=instance_shapes[k]) for k in instance_keys
        }
        ...
        for i in range(batch_size):
                ...
                if instance_info is not None:
                    ...
                    if self.anchor_labeler is None:
                        if i == 0:
                            ...
                            target_tensor = torch.full(
                                shape, instance_info["fill"], dtype=torch.float32
                            )
                    ...
                    if i == 0:
                        ...
                        if isinstance(tv, (tuple, list)):
                            ...
                            dtype = (
                                torch.float32
                                if isinstance(tv[0], (float, np.floating))
                                else torch.int32
                            )
                        else:
                            ...
                            dtype = (
                                torch.float32
                                if isinstance(tv, (float, np.floating))
                                else torch.int64
                            )
            ...
            if self.anchor_labeler is not None:
                (
                    ...
                    box_targets,
                    num_positives,
                ) = self.anchor_labeler.label_anchors(
