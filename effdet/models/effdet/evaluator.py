...
class TfmEvaluator(Evaluator):
    ...
    def evaluate(self):
        if not self.distributed or dist.get_rank() == 0:
            ...
            if self.distributed:
                dist.broadcast(
                    torch.tensor(map_metric, device=self.distributed_device), 0
                )
        else:
...
class OpenImagesEvaluator(TfmEvaluator):
    def __init__(self, dataset, distributed=False, pred_yxyx=False):
        super().__init__(
            ...
            distributed=distributed,
            pred_yxyx=pred_yxyx,
            evaluator_cls=tfm_eval.OpenImagesDetectionEvaluator,
        )
...
def create_evaluator(name, dataset, distributed=False, pred_yxyx=False):
    ...
    if "coco" in name:
        ...
        return OpenImagesEvaluator(
            dataset, distributed=distributed, pred_yxyx=pred_yxyx
        )
    else:
